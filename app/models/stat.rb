...
class Stat < ActiveRecord::Base
  ...
  def set_progress_level
    ...
    # Keys: type of experience, Values: strings w/ number of years of experience
    experience = {:mus => user.music_experience, :clin => user.clinical_experience, :gen_ed => user.general_education}
    total_experience = 0 # sum of all years of experience
    ...
    end # assigns the user to an initial "progress level" for progress tracking
  ...
  def update_experience(new_scores)
    # current score is a string formatted as "num_correct/num_total"; split by delimiter
    ...
    unless total_num.eql?(0)  # avoid dividing by 0 & reassign progress level
      if (correct_scores.to_f * 100 / total_num).round(2) > 60
        self.progress_level = "intermediate"
      else
        self.progress_level = "beginner"
      end
      self.total_answers = "#{correct_scores}/#{total_num}" # add new scores to current scores
      self.save
    end
  end
end
